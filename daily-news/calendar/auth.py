#!/usr/bin/env python3
"""Run once: get read-only Google Calendar consent in the browser and save token.json.

No dependencies. Put credentials.json {"client_id": "...", "client_secret": "..."}
into CALENDAR_CONFIG_DIR (default ~/.config/news-gcal) first. Create the OAuth client
...
CONFIG_DIR = Path(os.environ.get("CALENDAR_CONFIG_DIR", Path.home() / ".config" / "news-gcal"))
